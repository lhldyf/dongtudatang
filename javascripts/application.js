...
function vote3(id, v){
  ...
  // just for the basketball-vote page
  if(v==2){
    var word = $('#up-'+id).find('input');
    word.attr('value',"已戳");
  }
  //
...
$(function(){
    $('#user_login').change(function(){
           ...
           function(text){$('#insertlogin').html("<span style='color: red;'>"+text+"</span>");});
           }
 ...
 $(function(){
    ...
    var email=$('#user_email').val();
       if(isEmail(email)){$.get("/users/check_email",{user_email:$(this).val()},
           function(text){
             /*  if (text!="can_use"){ */
               $('#insertemail').html("<span>"+text+"</span>");
               
       });
         }
       else
           {$('#insertemail').html("<span style='color: red;'>请输入正确的邮箱地址</span>");
           }
